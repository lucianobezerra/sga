...
<html>
  <head>
    <script type="text/javascript">
      function desativar(id){
        $.post('model/solicitante.php?acao=desativar',
        {id: +id, ajax: true},
        function(resposta){
          if(!resposta){
            $('#row_'+id).fadeOut('slow');
            $('#retorno').html('Solicitante Desativado!');
            $('#retorno').fadeIn('slow');
      ...
      function excluir(id){
        $.post('model/solicitante.php?acao=excluir',
        {id: +id, ajax: true},
        function(resposta){
          if(!resposta){
            $('#row_'+id).fadeOut('slow');
            $('#retorno').html('Solicitante Excluido');
            $('#retorno').fadeIn('slow');
            $('#retorno').delay(4000).fadeOut(1000);
          } else {
            $('#retorno').html(resposta);
      ...
      $(function($){
        ...
        $('.novo_solicitante').click(function(){
  ...
  <body>
    ...
    <table width="100%" border="1" align="center" style="margin-top: 8px;font-size: 10pt; line-height: 130%">
      <thead>
        <tr>
          <th colspan="7" style='text-align: center; font-size: 12pt'>Solicitantes Ativos</th>
        </tr>
        <tr>
          <th style="width: 20%">CNS</th>
          <th style="width: 60%">Nome do Solicitante</th>
          <th colspan="4" style='text-align: center; width: 20%'>Ação</th>
        </tr>
      </thead>
      <?php while ($linha = $solicitante->retornaDados()) { ?>
        <tr id="row_<?= $linha->id; ?>">
          <td style='text-align: center;'><?= $linha->cns ?></td>
          <td><?= $linha->nome ?></td>
          <td style='text-align: center;'><?php echo ($nivel <2) ? "<a class='alterar' href='views/solicitantes/alterar.php?id={$linha->id}' title='Alterar'><img src='imagens/alterar.gif' border='0' alt=''/></a>" : "<img src='imagens/alterar.gif' border='0' alt='' title='Sem permissão para alterar'/>" ?></td>
    ...
    <br/><? echo ($nivel < 2) ? "<a class='novo_solicitante' href='views/solicitantes/cadastro.php'>Novo Solicitante</a>" : ""; ?>
  </body>
</html>